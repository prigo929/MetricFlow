...
*   **Friendly server pre-check**: `createOrder` (in `src/actions/index.ts`) re-queries the live `stock_qty` for every product referenced by the order — aggregating the requested quantity per product across all lines — and returns a descriptive message ("3 available, 5 requested") before attempting the write. This handles the common case with a clear error.
*   **Authoritative atomic write (`create_order_atomic` RPC)**: The client and pre-check are both read-then-act and subject to race conditions (two reps ordering the last units simultaneously). The actual write therefore runs through a PL/pgSQL function (migration `005_create_order_rpc.sql`) inside a single transaction: it inserts the header, inserts each line, and **decrements stock with a guarded, row-locking `UPDATE ... WHERE stock_qty >= qty`**. The row lock serialises concurrent orders for the same product, and if stock runs out the function raises and the entire transaction — header, prior decrements and audit rows — rolls back. This guarantees no overselling and never leaves an orphan order header.
...
*   **Form Mutations**: Instead of exposing raw API endpoints, our actions like `createOrder` receive validated form payloads, run their database writes, and output standardized `ActionResult` payloads back to client states. Multi-step writes that must be all-or-nothing (order header + line items + stock decrement) are delegated to a single PL/pgSQL function (`create_order_atomic`) so they execute in one database transaction; `total_amount` is then maintained by the `sync_order_total` trigger.
...
4.  **RLS Recursion Fix**: [supabase/migrations/004_fix_rls_recursion.sql](file:///Users/alinprigoreanu/Documents/Bachelor's%20Thesis/MetricFlow/supabase/migrations/004_fix_rls_recursion.sql) — defines the `get_user_role` helper and rewrites the role policies to avoid infinite recursion (must run after 001–002).
5.  **Atomic Order RPC**: [supabase/migrations/005_create_order_rpc.sql](file:///Users/alinprigoreanu/Documents/Bachelor's%20Thesis/MetricFlow/supabase/migrations/005_create_order_rpc.sql) — `create_order_atomic` (transactional order write with stock decrement).
